  ...
  const analyzeSymptoms = async (symptoms: string, age?: number, gender?: string) => {
    ...
    try {
      ...
      if (apiError.message?.includes('CORS') || apiError.message?.includes('cors')) {
        ...
        setError("Dữ liệu triệu chứng không hợp lệ. Vui lòng kiểm tra lại.");
      } else if (apiError.code === 'NETWORK_ERROR' || apiError.message?.includes('fetch')) {
        setError("Lỗi kết nối mạng. Vui lòng kiểm tra kết nối internet và thử lại.");
      } else {
        setError(apiError.response?.data?.message || "Có lỗi xảy ra khi phân tích triệu chứng. Vui lòng thử lại.");
      }
    } finally {
      setIsLoading(false);
    }
