...
// Thêm interface cho cart item nếu chưa có
interface ICartItem {
  _id: string;
  thumbnail: string;
  name: string;
...
const MedicineInfo: React.FC<MedicineInfoProps> = ({
  ...
  // Sửa lỗi: thay "onst;" bằng mutation hook
  const mutation = useMutation({
    mutationKey: ["add-to-cart"],
    mutationFn: (cartData: ICart) => addToCartAPI(cartData),
    onSuccess: () => {
      toast.success("Dã thêm vào giỏ hàng")
      router.push("/cart")
      // if (res?.message === "Add Product On Cart Successful") {
      //   toast.success("Đã thêm vào giỏ hàng!");
      //   router.push("/cart");
  ...
  return (
    <div className="flex-1 space-y-1">
      ...
      <p className="text-2xl text-red-600 font-bold">{sellingPrice?.toLocaleString()}VND</p>
      ...
      <div>
        ...
        <button
          ...
          className="bg-white border border-blue-900 text-blue-900 py-3 px-6 rounded-lg hover:bg-blue-900 hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
