...
export default function OrdersPage() {
  ...
  const getEmptyStateMessage = (status: string) => {
    const statusLabels: { [key: string]: string } = {
      "all": "đơn hàng nào",
      "Pending Confirmation": "đơn hàng chờ xác nhận",
      "Awaiting Shipment": "đơn hàng chờ giao hàng", 
    ...
    return {
      title: `Không có ${statusLabels[status] || "đơn hàng"}`,
      description: searchTerm
        ? "Không tìm thấy đơn hàng phù hợp với từ khóa tìm kiếm"
        : `Chưa có ${statusLabels[status] || "đơn hàng"} nào trong hệ thống`,
    };
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Error Alerts - Only show critical errors, not empty states */}
        {((ordersError && orders === undefined) || (statsError && stats === undefined)) && (
          <Alert className="mb-6 border-red-200 bg-red-50">
            <AlertCircle className="h-4 w-4 text-red-600" />
            <AlertDescription className="text-red-800">
              {ordersError?.message ||
        ...
        >
          ...
          {["all", ...ORDER_STATUSES.map(s => s.value)].map((tabValue) => {
            // const isAllTab = tabValue === "all";
            // const statusInfo = isAllTab ? null : ORDER_STATUSES.find(s => s.value === tabValue);
            const emptyState = getEmptyStateMessage(tabValue);
