...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Lỗi khi lấy danh sách bệnh",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Lỗi khi tạo bệnh'
    }, { status: 500 });
  }
}
...
//         process.env.MONGODB_URI || "mongodb://localhost:27017/medical_db"
//       );
//     }
