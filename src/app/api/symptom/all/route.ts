...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Lỗi khi lấy danh sách triệu chứng'
    }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newSymptom = new Symptom({ name, kindOf, symptomGroup });
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Lỗi khi tạo triệu chứng'
    }, { status: 500 });
  }
}
