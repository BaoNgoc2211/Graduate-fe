...
export default function OrderSuccessPage() {
  ...
  useEffect(() => {
    const getOrderInfo = (): OrderInfo | null => {
      // Kiểm tra từ URL parameters trước (cho payment callback)
      const urlParams = new URLSearchParams(window.location.search);
      const vnpayResponseCode = urlParams.get('vnp_ResponseCode');
      const momoResultCode = urlParams.get('resultCode');
      
      // Nếu có payment callback
      if (vnpayResponseCode || momoResultCode) {
        const isSuccess = vnpayResponseCode === '00' || momoResultCode === '0';
        ...
        if (isSuccess) {
          ...
          if (pendingPayment) {
            try {
              ...
              return {
                orderId: payment.orderId || `ORD-${Date.now().toString().slice(-6)}`,
                paymentMethod: vnpayResponseCode ? "VNPAY" : "MOMO",
                success: true
              };
            } catch (error) {
      ...
      // Kiểm tra từ localStorage (cho COD hoặc direct access)
      const orderSuccess = localStorage.getItem("orderSuccess");
      if (orderSuccess) {
        try {
          return JSON.parse(orderSuccess);
        } catch (error) {
          console.error("Error parsing order success:", error);
        }
      }

      // Fallback từ checkout session
    ...
    if (!info) {
      // Nếu không có thông tin order, chuyển về cart
      router.push("/cart");
      return;
    }
    ...
    // Trigger confetti animation chỉ khi thành công
    if (info.success) {
      const duration = 3000;
    ...
    // Clean up localStorage sau một thời gian
