...
const PriceRow = ({ label, value, isTotal = false, isDiscount = false }: PriceRowProps) => (
  <div className={`flex justify-between ${isTotal ? 'text-lg font-semibold' : ''} ${isDiscount ? 'text-green-600' : ''}`}>
    <span className={isTotal ? 'text-gray-900' : 'text-gray-600'}>{label}</span>
    <span className={isTotal ? 'text-blue-900' : 'font-medium'}>
      {typeof value === 'number' ? (
        isDiscount ? `-${formatPrice(value)}` : formatPrice(value)
      ) : value}
    </span>
  </div>
);
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          ...
          <PriceRow 
            ...
            value={shippingPrice === 0 ? "Miễn phí" : shippingPrice}
          />
        ...
        <div className="text-center">
          <p className="text-xs text-gray-500">🔒 Thông tin của bạn được bảo mật an toàn</p>
        </div>
      </CardContent>
    </Card>
  );
