...
// Update prescription data interface
...
export function ChatBox({
  ...
  const { currentRoom, messages, isLoading, isSending, isConnected, startChat, sendMessage, refreshMessages } = useChatSocket({
    userId,
    onNewMessage: () => {
      setTimeout(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" })
  ...
  const handleUpdatePrescription = async (updateData: UpdatePrescriptionData) => {
    try {
      console.log("Updating prescription:", updateData)
      // Here you would call your update prescription API
      // For now, we'll just send it as a regular message
      const medicineText = updateData.medicines
        .map((med: IMedicine) => `${med.name} ${med.dosage} - Số lượng: ${med.quantity}`)
        .join(", ")
