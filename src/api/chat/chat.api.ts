...
// };
export const getUnassignedRooms = async (): Promise<{ data: IChatRoom[] }> => {
  const res = await APIConfig.get<{ data: IChatRoom[] }>(`/api/chat/unassigned`);
  return res.data;
};
...
export const getMessages = async (
  ...
  const res = await APIConfig.get<{ data: IMessage[] }>(`/api/chat/messages/${roomId}`);
  return res.data;
};
...
export const sendMessage = async (
  ...
  const res = await APIConfig.post<{ data: IMessage }>(
    `/api/chat/send`,
    data,
    { withCredentials: true }
  );
