...
export const getProfileAPI = async () => {
  const response = await APIConfig.get("/api/auth/profile");
  return response.data;
};

export const updateInfoAPI = async (data: IInfo) => {
  const response = await APIConfig.put("/api/auth/profile", { info: data });
  return response.data;
};

// Fixed: Remove :id and change to POST
export const forgotPasswordAPI = async (data: { email: string }) => {
  const response = await APIConfig.post("/api/auth/forgot-password", data);
  return response.data;
};
