...
// // Get order by ID - Call detail API và combine với list API nếu cần
// export const getOrderByIdAPI = async (order_id: string): Promise<IOrder> => {
//   try {
//     console.log(`🔍 Getting order details for ID: ${order_id}`);
    
//     // First try to get from list API to get basic order info
//     const allOrders = await getAllOrdersAPI();
//     const orderFromList = allOrders.find(order => order._id === order_id);
    
//     if (!orderFromList) {
//       throw new Error(`Order with ID ${order_id} not found`);
//     }
    
//     // Then get detailed info from orderdetail API
//     try {
//       const detailResponse = await APIConfig.get<{ data: APIOrderDetailResponse }>(
//         `/api/order/orderdetail/${orderFromList.orderDetailId || order_id}`
//       );
...
//           totalAmount: item.totalAmount || 0,
//           note: item.note,
//         })) || orderFromList.orderItems,
//         createdAt: detailResponse.data.data.createdAt || orderFromList.createdAt,
//         updatedAt: detailResponse.data.data.updatedAt || orderFromList.updatedAt,
//       };
//     } catch (detailError) {
//       console.warn('Could not get order details, using list data only:', detailError);
...
// // Map order list response từ backend với đầy đủ thông tin user
// function mapOrderFromListAPI(apiOrder: APIOrderListResponse): IOrder {
//   console.log("🔍 Mapping order from list API:", {
//     orderId: apiOrder.orderId,
//     status: apiOrder.status,
//     itemsCount: apiOrder.items?.length || 0,
//     userInfo: apiOrder.userId?.info
//   });
...
//       email: "", // Không có trong API response
...
//     })) || [],
//     totalAmount: apiOrder.totalAmount || 0,
//     shippingFee: (apiOrder.finalAmount - apiOrder.totalAmount) || 0, // Tính từ difference
//     discount: 0,
//     finalAmount: apiOrder.finalAmount || 0,
//     status: mappedStatus as IOrder["status"],
//     paymentMethod: "COD",
...
//     createdAt: new Date().toISOString(),
//     updatedAt: new Date().toISOString(),
//     orderDetailId: apiOrder.orderDetailId, // Thêm field này để reference
//   };
// }
...
// Map order detail response from backend (kept for compatibility)
// function mapOrderFromDetailAPI(apiOrderDetail: APIOrderDetailResponse, orderId: string): IOrder {
//   console.log("🔍 Mapping order from detail API:");
//   console.log("📦 Detail API data:", JSON.stringify(apiOrderDetail, null, 2));
...
//     },
//     orderItems: apiOrderDetail.order_items?.map((item, index) => {
//       console.log(`🏷️ Processing detail item ${index}:`, item);
//       return {
//         _id: item.medicine_id || `item-${index}`,
//         medicine_id: {
//           _id: item.medicine_id || "unknown",
...
//     discount: 0,
//     finalAmount: apiOrderDetail.totalOrder || 0,
//     status: "Pending Confirmation", // Not available in orderDetail API
//     paymentMethod: "COD",
//     shippingMethod: "Standard Shipping",
//     shippingAddress: {
//       name: "",
